...
## 2026-02-22 — version 1.1.5

### Platform support

**macOS compatibility**
- `mux/pty_helpers.h`: replaced the unconditional `#include <pty.h>` with a
  platform guard — on Apple targets (`__APPLE__`) the header is now
  `<util.h>`, which provides `openpty` and friends on macOS; all other
  platforms continue to use `<pty.h>`.
- `readline_fix.v`: introduced a new `VlshReadline` struct that embeds the
  stdlib `Readline` and carries the Ctrl+R history-search fields
  (`search_query`, `search_match_index`, `search_mode`,
  `search_saved_prompt`, `search_saved_prompt_offset`,
  `search_saved_current`) directly. The vlib `Readline` struct omits these
  fields on some platforms (including macOS), so keeping them in `VlshReadline`
  ensures the code compiles and runs correctly on all supported targets.
- All internal functions that previously accepted `Readline` (or `mut Readline`)
  now accept `VlshReadline` (or `mut VlshReadline`).
- `vlsh.v`: the `import readline { Readline }` import is removed (no longer
  needed); `main()` instantiates `VlshReadline{}` instead of `Readline{}`;
  `load_history` updated accordingly.

### Version bumps
- Version bumped to `1.1.5` in `vlsh.v`, `v.mod`, and `README.md`.
...
- An alias whose expansion begins with a shell builtin (e.g. `~local` → `cd ~/.local`)
...
- New `run_output_hooks(loaded []Plugin, cmdline string, exit_code int,
  output string)` function in `plugins/plugins.v`; invoked from
...
- `help plugins` updated: removed `plugins remote list` / `plugins remote
  search`; added `plugins search <query>` and `plugins update [name]`;
...
- `README.md` updated with install options for pre-compiled packages
  (`.deb` and standalone binary), including a new *Installation* section
  listing both build-from-source and pre-built package methods.
- Removed the TODO section from `README.md`.
